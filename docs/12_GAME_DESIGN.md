...
變多、分流與延遲，但同屏總數最多 6 個。持有神賜時具體物件取代通用白色劍氣幾何，讓普通攻擊的
出現方向、飛行路徑、成長、變色和消失方式產生質變，避免再次看成快速噴出的一坨素材。
...
羽毛是神聖羽毛、巨盾是王室盾牌、水流是海浪、火焰是流火。基本階只出現一個主體；
進階增加同主體數量並排成一條路徑；大師階再增加數量，拆成至少三條且三個方向的路徑。
