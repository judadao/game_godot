...
十字光芒等抽象幾何，而以火刃成長、影刃召回、毒種綻放、雷光閃現、羽扇展開、冰刃出土、
潮鏢迴旋與日輪天降，以及 10 種進化主體動作呈現元素差異，而不是只抽取數值屬性。各 profile
...
新融合的背景攻擊從 `size_scale 0.72`、單一具體主體開始；第一批 10 個融合分別使用天輪、
斬首冰刃、毒羽、潮槍、雙刀、疫冠、雷槍、戰馬、極光聖槍與死神收割。Combo Chain、另外兩格
神賜與所有神賜等級會共同提高尺寸、同屏 instance、三拍速度、target count、damage scale 與殘影；
高成長時形成多重交錯陣列。融合背景攻擊同屏最多保留 2 個 124–320px、至少演出 1.25 秒的清楚主體；
8 種 base 神賜也各自把 84–176px 的具體物件附到普通攻擊，從單一物件隨等級／Combo／stack 變大、
變多、分流與延遲，但同屏總數最多 6 個。持有神賜時仍保留白芯月牙劍氣作為普通攻擊主體；
神賜物件先沿各自方向與路徑展開，月牙再於命中前快速追上，兩者共同形成質變而非彼此取代。
神賜狀態同時套用到普通攻擊、所有傷害招式與背景攻擊，
治療／純輔助招式不觸發。
...
雷電系列的基本招式 `引雷一閃` 固定由 `風暴充能 ×3` 觸發；不能借用奧術／回聲的
混合配方，否則玩家明確編成雷電劍魂後仍無法從三段基本 Combo 叫出雷電。

本階段的動畫成長採 Skill VFX Grammar：13 系列各自只有一個可重複拼裝的 Core；具體
物件使用透明 bitmap，黑洞、火柱、殘雷、推浪、龍息、治療域與肉體光暈可明確採用
procedural Core。專用 renderer 至少拆成 Core／輪廓／動勢／粒子／爆點五層，Blessing
只改材質、顏色、數量、軌跡與附加層，不反向持有傷害規則。劍雨例外從
基本 10 把開始，進階 15 把、大師 20 把，每五把形成一輪。劍雨另採
...
具體主體並疊加能量材質、雙層弧形尾跡、清楚的雙層光輪與消散粒子。羽毛不飛向或追蹤
敵人；敵人 Hurtbox 碰到環身羽界時，每 0.18 秒承受一次持續切擊與從玩家向外的擊退。
3／7／15 羽分別維持約 4.8／6.4／8.8 秒，數量越多持續越久；末段從外圈至內圈逐根
消失。同系列招式在消散前再次施放會刷新既有接觸領域、補回羽毛並重新逐根浮現，不可
重疊生成另一個光輪或改成追蹤彈幕。
月輪 basic／advanced／master 固定為 5／8／12 枚，分別完成 1／2／3 次完整往返；
月輪不是沿同一條弧線同步擺動，而是由兩側錯相釋放，在畫面上下與左右邊界做分離的
彈珠式折射。每次去程與回程都是命中節點，同一趟的多枚命中以短間隔依序落下；可見
殘影只保留輪體後方 22／44px 的兩個短材質回聲，每枚月輪各自攜帶貼身能量光與較大的外層 bloom，
兩層都必須跟著該輪移動並於反彈點增亮；禁止顯示完整路徑線。火焰固定由地面向上分段噴發
5／10／20 柱；雷電在
10／20／30 名敵人間反彈留下殘雷，倒數後逐一降下更強天雷。殘雷以較寬、半透明的
多段局部電弧在被標記敵人身體外圍快速竄流，不能遮住敵人輪廓，也禁止用一條常駐折線把所有目標串起來；chain hop
只在真實命中時顯示極短 A→B 導電頭。天雷則依序顯示預充電、天空主雷柱、白熱核心、
分支、接地曝光、火花與地表殘電；主雷必須從可視戰場上緣外由上往下劈落，不能只在
敵人頭頂短距離整條閃現。水流是持續傷害的外推浪，
總位移上限 140px。龍息為 1 路 180 度掃蕩、2 路左右掃蕩，以及大師左右掃後追加上方
20 龍首向下噴吐 1.5 秒。朝陽生息是跟隨玩家的綠金治療域；同枝共生則是肉體強化，
大師維持 7 秒、移速 1.8 倍、攻速 1.75 倍並顯示八層彩色殘影。
...
Composer 重複渲染。
所有系列的材質與命中層採漸進式品質順序：先保證主物體／場域剪影能辨識，再校準
anticipation、release、contact、afterbeat 的玩法節奏，接著才增加材質流動、粒子密度與
glow。實際命中層必須由 controller signal 觸發，不可只在 VFX normalized timeline 的固定
百分比假裝命中。每個命中以短白熱 contact flash、元素主形擴張、方向性碎屑、煙霧／水霧／
腐蝕／殘光收束四拍呈現；不同層不得同時放大又同時消失。
...
九元素並可投影目前裝備武器的原初屬性。武器原初屬性與已取得的祝福屬性會一起套用
到攻擊，屬性之間完全沒有增傷相剋、弱點或抗性減傷，只疊加各自的附加效果：

| 屬性 | 附加效果 |
|---|---|
| 水 | 潮濺：額外波及 1 個目標；貫穿劍氣加寬 20% |
| 火 | 灼燒：短時間火焰持續傷害 |
| 風 | 風壓：擊退力提高 30% |
| 雷 | 感電：命中短暫暈眩 |
| 冰 | 寒意：降低移動速度 15%，持續 1.5 秒 |
| 毒 | 中毒：3 秒毒性持續傷害 |
| 光 | 輝癒：依命中總傷害恢復 3% 生命 |
| 暗 | 蝕命：4% 傷害生命竊取 |
| 普通 | 真鋒：暴擊率提高 4% |
...
只有覺醒圖紙能產生傳奇品質。覺醒時預設為衡鍛流；玩家可在對應圖紙商支付
120 gold 與 8 magic shards 改為銳鋒流、省材流、名印流或共鳴流，改造不重置熟練度。

鍛造前必須選擇工法：穩鍛為均衡基準、精煉增加素材與加工費換取成功率／品質、
急鍛降低費用但提高瑕疵與廢料風險、名匠鍛造只供覺醒圖紙使用並可產生意外神作。
配方中用量最高的素材提供韌性木紋／沉穩石質／導魔結晶／活性核心其中一項特性，
實際修正成功率或品質。失敗不只顯示失敗字樣，而會結算瑕疵品、試作品、回收廢料
或意外神作；回收廢料返還該次基礎素材的 25%。
...
- Player Blacksmith：進場先以舊工業鐵匠鋪圖層的熔爐、工作台與商店門呈現可互動工坊，
  三個圖像 hotspot 可用方向鍵／Tab 聚焦，選取物件後才開啟
  ...
  玩家可升級 Sword Soul，並從鐵匠鋪進入獨立玩家商店：置中 1040×640 authored frame 會依
  viewport 等比縮放至 0.78×–2.0×，frame 上方有
  暖木、花卉與農產構成的溫馨店景，以及主角、顧客、櫃台、招客鈴與實際陳列商品；
  兩位大型既有 NPC 會沿地板路線逛店、停留並輪替來去。初始不列管理功能，點商品才在左側
  overlay `ContextDock` 顯示庫存／定價且不壓縮店景，
  ...
  的精品價不會被一般顧客接受。流言菲語會公開特定商品／最低品質需求；符合時具名
  顧客必定來訪，再套用 170%–220% 的高價倍率。Market 裝潢與商旅印章則建立一般
  高價客群。顧客拒買後商品仍留在架上，由後續顧客繼續評估，不需要玩家手動結帳。
- Market：未擴建時可先販售戰鬥素材並使用木製交易台的兩格；Level 1 開放裝備買家、
  雪松展示櫃與 Tier 2 圖紙，Level 2 開放鍛鐵陳列櫃與 Tier 3 圖紙，Level 3 開放
  大市集交易櫃。建築升級不直接贈送家具，且家具必須逐階購買；
  三級累計提供 10% 購買折扣與 20% 販售加成。
...
- Sword Soul Shop：buy-only 圖紙 catalog；持有後不能重複購買；已覺醒圖紙可在此
  支付改造費切換流派。
- Equipment Blueprint Shop：基礎普通裝備直接販售；其餘只販售不可重複購買的圖紙，
  高階成品必須回玩家工坊鍛造；已覺醒圖紙可在此支付改造費切換流派。
