...
| Specialized series material renderers | `scenes/vfx/skills/`、`scripts/vfx/*material_vfx_2d.gd` | 將上述系列拆成至少五個可讀層；`LayeredSeriesMaterialVFX2D` 共用能量輪廓、動勢線、accent、粒子與 procedural Core，Blessing 可改 palette、密度、軌跡與爆點。月輪另由 `MoonWheelBounceMaterialVFX2D` 以相反起始方向、上下／左右邊界的分離彈珠反射、每輪各自跟隨的貼身 shader aura＋外層 bloom、短材質殘像、接觸壓縮、月光碎片與粒子頓點同步具體環刃；不建立任何可見的完整軌跡線 |
