...
合併進真實 Finisher 結算；`CardEffectRunner` 只建立系列的 runtime profile，持續行為由
各自 controller 執行：月輪往返、環身羽界、荊棘花庭、石之無人機、黑洞、密術沼澤、
分段火柱、殘雷天擊、傷害推浪、龍息掃蕩、跟隨治療域與肉體強化。圖鑑必須先說明玩家
如何走位／選擇時機，再描述招式演出。
自動招式只改變攻擊物件、目標、軌跡與效果，禁止修改玩家座標。

`Game._on_card_effect_resolved()` 是持續系列 gameplay controller 的 composition root。
Controller 只擁有命中、時間、位移／治療／Buff；`NamedSkillVFX` 的專用 renderer 只擁有
Core、Trail、Aura、Ground、Particle 與 Impact 等畫面層。兩者不得互相代算傷害或視覺。
`SeriesImpactVFXRouter` 是兩者之間唯一的 presentation adapter：只監聽 controller 已確認的
噴發、彈射、命中、墜毀、治療 pulse 與 detonation signal，再於 `current_map` 生成共用
Fire／Lightning／Water／Poison／Wind primitive；不重新找目標或計算傷害。Blessing overlay
只投影 primitive palette、noise、particle density 與 glow。
同枝共生的暫時攻速由 `PlayerController` 持有，`Game` 僅以該倍率縮短下一次自動攻擊間隔；
狀態到期即自動回到 1.0，不永久改寫卡牌資料。
...
| `SeriesImpactVFXRouter` | `scripts/vfx/series_impact_vfx_router.gd` | 將 persistent series controller 的真實 signal 轉成可重用材質／命中原語；四拍依序為 contact flash、shape expansion、secondary debris、residual fade。雷電 `chain_hit` 只生成 A→B bolt，`final_strike` 才生成 top-down impact。只生成 presentation，禁止自行判斷命中 |
...
| Specialized series material renderers | `scenes/vfx/skills/`、`scripts/vfx/*material_vfx_2d.gd` | 將上述系列拆成至少五個可讀層；`LayeredSeriesMaterialVFX2D` 共用能量輪廓、動勢線、accent、粒子與 procedural Core，Blessing 可改 palette、密度、軌跡與爆點。月輪另由 `MoonWheelBounceMaterialVFX2D` 以相反起始方向、上下／左右邊界的分離彈珠反射同步具體環刃。雷電另由 `ResidualLightningMaterialVFX2D` 透過 presentation-only position provider 每幀跟隨被標記物體，以 1.45 倍輪廓範圍、0.68 opacity 的多段局部竄流保留敵人可讀性；顯示目標沿用 gameplay 的敵人節點距離排序，再把 Hurtbox 中心以 `to_local()` 轉換成出圖位置，確保 target limit 一致且翻面／縮放後不漂移。跳焦 spark 使用按正式 hop interval 計算的 bounded world-space emitter pool，不畫 caster／target／target 之間的常駐路線，真正 chain／sky strike 僅由 controller signal 交給 primitive 呈現；sky strike 從 viewport 上緣外起始，至少跨 620px 由上往下揭露，接地後才觸發 flash、spark 與殘電 |
...
第 4 層為 1.5 秒，之後每層減少 0.1 秒，最低 0.6 秒；此基線讓 2 AP 卡在無裝備時
仍可完成六段大師路線，專注護符最後再加上 0.5 秒。
Combo 倒數與 AP 回復都使用 real-time delta，hit stop／戰術慢動作不得讓 AP 相對變慢，
否則六段大師路線會在視覺停頓期間變成不可完成。
...
- 新招式的傷害、AP、解鎖、施放與升級規則尚未核准，composition root 不得從名稱
  ...
  保留並回傳空結果。
- `skill_series_vfx.json` 是現役招式 presentation 權威：bitmap Core 與明確標記的
  procedural Core 都必須有唯一 series profile。一般發射型系列維持多主體可讀性；月輪固定
  5／8／12 枚與 1／2／3 次往返，場域與持續型系列依其 controller 數量、半徑、時間成長。
  劍雨 profile 另外定義環繞浮現、0.8 秒垂直鎖定、
