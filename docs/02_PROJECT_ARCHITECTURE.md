...
補足西側、內側與東側可見樹叢；一張 3 × 2 atlas 提供六種通用手繪葉團，
另以 2 × 2 atlas 提供四種圓鈍垂落葉袋，專門打破西側樹冠的水平底緣。
兩者都沒有樹幹、枝根或完整樹形，並疊進不移動的原始秋林底圖。
西側材料行樹另有一個 `z=-20` 靜態分枝／樹幹錨點，底部嵌入屋頂後方；
動態葉袋位於 `z=-21` 並只在兩個局部 pivot 上擺動。樹幹不參與 sway，
`z=-10` 前景屋頂遮住底部接縫，使橘色樹冠能在古塔前可見但不成為無根的浮空葉帶。
一般狀態下八組中央樹冠以不同週期低幅擺動並穿插稀疏抖葉，覆蓋可見的上冠、
左右外冠、中冠與前後冠。十組街景 foliage patches 以不同週期在隱藏支點上
低幅旋轉；西側兩個 pivot 可各由主冠與較低的小葉袋共同組成，但同一 pivot
內只形成局部枝端，並由相鄰 pivot 提供相位差。原始背景樹、樹幹與整棵樹的位置保持固定，
不使用 UV 水波、整片平移或伸縮。每隔 8–18 秒再由
陣風讓不同相位的樹冠模組明顯受風再回正。落葉全部從中央古樹
