...
- Player Blacksmith：進場先以舊工業鐵匠鋪圖層顯示熔爐、工作台與商店門，不能把服務功能直接列成選單；
  ...
  店內使用 1040×640 authored service frame；以 1280×720 為 1× 基準隨 viewport 在
  `0.78–2.0×` 間等比縮放並保持置中。上方緊湊狀態列呈現櫃台、陳列商品、主角店主、
  顧客與入口；店景採暖木、花卉、農產與日光的溫馨雜貨店前中後景。初始只顯示店內可互動物件，
  不能直接列出庫存／貨架／流言功能。點商品展示位才在店景左側顯示小型 `ContextDock`，
  點牆面貨架只顯示貨架管理，點招客鈴才顯示流言資訊；dock 是 overlay，開關時不得壓縮或移動店景。
  顧客使用既有 Town NPC，以足部對齊地板的大尺寸角色沿兩條路線逛展示桌、花架與櫃台，會在節點停留、
