...
- `PlayerBlacksmithWindow` 使用 1180×660 authored frame，依 viewport 等比縮放並置中。
...
- Player Blacksmith：進場以單張整合式中世紀工坊場景 `blacksmith_hub_background_v2.png` 顯示同一房間內的三座門廳，分別以爐火鐵砧、設備工作間與武器素材店表示鍛造、升級與販賣；不能把服務功能做成三張資訊卡或直接列成選單；
  三個圖像 hotspot 必須能用方向鍵／Tab 切換，並保留可見焦點框；
  三個透明 hotspot 必須貼合場景中的門廳，底部以繁中名稱與單行用途說明；焦點使用暖金細亮框，
  不得以實心遮罩蓋住場景物件，也不得再疊貼另一套物件 cutout；
  正式鍛造使用 `forge_workspace_objects.png` 的圖紙架、工具架、素材箱、鐵砧／槌子與成品架。畫面不能同時攤開完整流程：每個狀態只顯示當前可互動的一個工作物件，觸碰後才展開該次的單一決策；完成選擇就收合並切換到下一個物件。圖紙清單、鍛法、素材品質、鐵砧與成品操作不得同時可見；詳細機率保留於 tooltip／次要說明，不與單一動作提示搶視線；
  圖紙列第一次按下只更新預覽；同頁唯一暖金主按鈕必須明寫「確認圖紙：名稱」，確認後才收起清單、顯示工具架並把鍵盤焦點移到下一個物件；
  ...
  店內使用 1180×660 authored service frame；以 1280×720 為 1× 基準隨 viewport 在
  `0.78–2.4×` 間等比縮放並保持置中。上方緊湊狀態列呈現櫃台、陳列商品、主角店主、
  顧客與入口；店景是低亮度的中世紀武器／防具／素材店，以灰藍石牆、深木、鐵件、武器牆、
  盔甲架與素材抽屜形成清楚前中後景，不使用花店、農產店或過曝日光語彙。初始只顯示店內可互動物件，
  不能直接列出庫存／貨架／流言功能。點商品展示位才在店景右側顯示小型 `ContextDock`；
  補貨 dock 依序只揭示商品、價格、確認三個當前決策，不得一次攤開完整表單。選定商品後收起
  商品清單並顯示價格，選定價格後只留下單一確認上架操作；商品與價格階段必須提供「上一步」，
  `ui_cancel` 在 dock 開啟時也先逐層返回／收起，不得直接把玩家送出店外；
  點牆面貨架只顯示貨架管理，點招客鈴才顯示流言資訊；dock 是 overlay，開關時不得壓縮或移動店景。
  顧客使用既有 Town NPC，以足部對齊地板的大尺寸角色沿兩條路線逛武器架、素材櫃與櫃台，會在節點停留、
  從入口淡入淡出並於下一圈輪替角色，且自動
  ...
  店內視覺使用灰藍石牆、深木樑、武器牆、盔甲架、素材抽屜與厚木結帳櫃台，以及可替換的
  銅鈴、武器工具、鐵匠徽旗、鋼錠素材箱、瓶架與盾甲展示。三層圖片皆為
  `MOUSE_FILTER_IGNORE`；hotspot 不得被前景阻擋，鍵盤 focus 必須清楚可辨。背景、前中景與裝潢
  必須沿用 Base MaterialYard 的大型色塊、粗斷線、有限色階與低細節頻率，禁止寫實高光與密集裂紋。
...
提示不得只寫抽象路線：每張候選卡在一般效果上方固定顯示融合狀態帶，明寫
「可融合｜你已持有『搭檔名稱』」或「目前持有神賜無法與這張融合」；缺少觸媒時
同一條狀態帶顯示「還缺觸媒名稱」，不要求玩家先選卡或開 tooltip 才能判讀。

合法融合必須顯示獨立的「昇華融合・專屬背景自動攻擊」section，不得混在一般升級列
或隱藏。每張融合卡直接顯示由兩個來源名稱組成的融合名稱、專屬背景攻擊名稱、interval、target count 與
「繼承劍魂」說明；完整 tooltip 仍保留 description。背景攻擊以該融合的天輪／冰刃／羽扇／
潮槍／雙刀／王冠／雷槍／戰馬／聖槍／死神具體主體為唯一第一閱讀層，不再疊加抽象幾何 motif。
主體依元素採放大、召回、土中冒出、天降、扇形、閃現、迴旋與變色消失等不同軌跡，不能退化成
只挑幾個數值屬性。合法融合超過三種時，每次隨機抽三種並優先
