...
`TownBattlePortalAnimation.tscn` 的十二幀漩渦。Portal root 固定在互動座標；
`ApertureAnchor` 依漩渦縮放補償底緣，將效果嵌進背景門洞且 ambient animation 不得修改
root、anchor 或 scale transform。可用狀態播放帶地區色的漩渦呼吸；封印狀態改用靜止暗色
漩渦、門洞內 veil、輪廓與小型 glyph，不使用旋轉雙環或交叉鎖印。
中央 Boss 的漩渦可依大門放大，但封印 veil 與 glyph 使用獨立縮放並保持在下方黑色門洞內，
不得顯示跨出石拱的大型外圈或 aura；Boss 可用狀態也只顯示門洞內的十二幀漩渦，
不顯示側門使用的 active aura 或頂部 rune。
`battle_portal_hub.gd` 只依 progression 切換同一元件的 `sealed` 狀態。
`StairWalkBaseline` 是角色與傳入 `PlayerSpawn` 的共同 `y=640` 權威；
`FloorCollision` 中心為 `y=720`、上緣為 `y=672`，讓物理更新後角色落在前景鋪面而非
背景中景樓梯。
