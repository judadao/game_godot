...
`card_growth_fusion_<width>x<height>.png`。融合頁必須人工確認獨立 section、專屬背景
攻擊名、interval、target count、繼承劍魂說明、selected／focus 差異與 `SelectionSummary`；1152×720 不得裁字，
...
| `town_building_ui_behavior_test.gd` | Material offer intent；blacksmith 升級／recipe intent；圖紙覺醒／傳奇機率與品質販售 row；Town Hall 精確升級；一般 Shop quantity 與圖紙商 buy-only |
...
| `battle_portal_hub_animation_test.gd` | 四地區與中央 Boss 門皆有自動播放的 Town-style 漩渦、封印鎖環狀態，以及 Player／spawn／樓梯共同 baseline |
...
| Survival pressure | 6:00、開場 30 隻；alive cap 48→170、batch 10→20、interval 0.35→0.08 秒；60 秒先出 0.72 HP／0.86 scale 小 Boss，90 秒啟動十秒 super horde，180 秒第二 Boss、300 秒起複數 Boss，最後 30 秒 Final Rush 達最大量；非 completion Boss defeat 啟動十二秒 rapid-level horde（cap +85、batch +34、interval ×0.15）；普通怪死亡後 0.05 秒內排入補怪 |
| Enemy durability/knockback | 普通怪 HP 倍率沿 360 秒 timeline 由 10.0 平滑提高到 26.0，傷害倍率 1.15→2.20；只有未死亡目標建立 0.16 秒 hit-knockback state，且 pursuit／navigation recovery 不得在下一幀覆寫水平擊退 |
| Run XP pacing | 普通怪的完整 XP 會拆成滿地單點 gem；90 秒 super horde 為 2 倍、Boss 後 rapid-level horde 為 3 倍；Gem 拋射後落地停留 0.65 秒才可吸取；Run 首級門檻 40，後續為 `ceil(previous × 1.20 + 12)`，445 XP 可排入前五個 level-up pages，大量 XP 仍以 while 完整入列 |
...
| Player Market experience/art | `player_market_experience_test.gd` 驗鐵匠三門廳鍵盤 focus、商店 Product1 初始焦點、右側 `ContextDock` 不壓縮店景、補貨商品→價格→確認逐步揭示、明確上一步返回與兩條顧客路線；`player_market_art_layers_test.gd` 驗 medieval armory background／midground／foreground、低亮度 tone grade、六個 atlas 裝潢與所有物件 hotspot；`blacksmith_shop_visual_upgrade_test.gd` 驗入口為單一整合式工坊場景而非三張卡、圖紙預覽後須按唯一「確認圖紙」CTA、其後每次只揭示當前物件與決策、鍵盤焦點自動前往下一站、generated menu icons 與鍛造／店內微動畫；`blacksmith_shop_layout_test.gd` 驗六個 required resolution，並可輸出工坊、店面、側 dock 全景與固定 3×2 六切片；鐵匠鋪 1180×660 authored frame 以 viewport 88% 為上限在 `0.78–1.9×` 等比置中，Player Market 維持自己的 viewport scaling contract；`blessing_market_visual_capture_test.gd` 保留戰鬥／普攻／地名整合 capture |
| Story NPC / OB icons / blue-flame Boss | `town_npc_story_animation_test.gd` 驗五名角色各 6 表情、6 動作與 4×6 alpha atlas；`ob_story_weapon_icon_test.gd` 驗八件 OB 代表武器與來源 mapping；`autumn_smoke_oni_animation_test.gd` 驗七種命名 puppet 動畫與正式拆件。設定 `GENERATED_STORY_CAPTURE_DIR` 執行 `generated_story_visual_capture_test.gd`，輸出 NPC、武器與 Boss 三張 1920×1080 full-frame 及每張固定 3×2 六切片供獨立視覺 review。 |
