...
| Fusion | 只由菁英／Boss loot page 提供第一批 10 個 authored recipe；非配方組合必須拒絕，5 個儀式配方另驗 equipped catalyst；合法配對超過三種時隨機抽三種且 queue 優先保留三個 fusion；背景攻擊保留兩個來源 motif／glow color，以粗外光暈、能量本體、近白核心及三拍聖環繪製；初始 `size_scale < 1`／單一 instance，Combo、支援神賜與等級使尺寸、數量、節奏、target 與 damage 一路放大；0 AP、不動 Basic cooldown，並讓普通攻擊、傷害招式與背景攻擊共用全部神賜狀態 |
