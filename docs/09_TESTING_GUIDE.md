...
| Deck/hand | 傳送門前第 1 格固定 Healing、後 3 格為 unique 公式劍魂；招式選擇自動填入 `required_skills` 聯集，相容名稱保持正常色、缺卡或超過三格者保留並反灰；手動候選排除重複並預覽終結技；圖示、類型色與金色幾何動畫可辨；hover／keyboard focus 即時預覽效果，向下越界自動捲動；面板於高解析度等比放大；QWER 使用後保留原 slot |
