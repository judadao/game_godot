...
| Player Market experience/art | `player_market_experience_test.gd` 驗鐵匠三熱區鍵盤 focus、商店 Product1 初始焦點、側邊 `ContextDock` 不壓縮店景與兩條顧客路線；`player_market_art_layers_test.gd` 驗 cozy background／midground／foreground、六個 atlas 裝潢與所有物件 hotspot；`blacksmith_shop_visual_upgrade_test.gd` 驗圖像物件、generated menu icons、可升級櫃台容量說明與鍛造／店內微動畫；`blacksmith_shop_layout_test.gd` 驗六個 required resolution，並可輸出工坊、店面、側 dock 全景與固定 3×2 六切片；1040×640 authored frame 以 1280×720 為 1× 基準在 `0.78–2.0×` 等比置中；`blessing_market_visual_capture_test.gd` 保留戰鬥／普攻／地名整合 capture |
