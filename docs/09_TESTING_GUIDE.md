...
| Survival pressure | 8:30、開場 30 隻；alive cap 48→170、batch 10→20、interval 0.35→0.08 秒；90 秒啟動十秒 super horde 並出第一 Boss、180 秒第二 Boss、300 秒起每波 2／2／3／4 隻，480 秒與 Final Rush 疊成最大量；非 completion Boss defeat 啟動十二秒 rapid-level horde（cap +85、batch +34、interval ×0.15）；普通怪死亡後 0.05 秒內排入補怪 |
| Enemy durability/knockback | 普通怪 HP 倍率沿 510 秒 timeline 由 10.0 平滑提高到 26.0，傷害倍率 1.15→2.20；只有未死亡目標建立 0.16 秒 hit-knockback state，且 pursuit／navigation recovery 不得在下一幀覆寫水平擊退 |
...
| Fusion | 只由菁英／Boss loot page 提供兩項不同 Lv.3 神賜融合；獨立 FusionSection 顯示專屬背景攻擊；材料退出獎勵池並生成 Lv.1 evolved gift；背景攻擊保留兩個來源的 geometry motif／glow color、獨立計時、0 AP、不動 Basic cooldown，並繼承目前劍魂傷害／元素／狀態效果 |
...
| Divine Gifts | catalog 固定 20 項且各有唯一繁中名稱、fusion stem／motif／色彩；每個 EXP level 必選新／升級；菁英／Boss 必選既有升級／融合；最多 4 slot；全 inventory 的中文前綴與 mechanics 依序累加；Lv.3 融合材料退出獎勵池並釋出一格；快節奏路線可將選定六項收束成三項 evolved，不要求八分鐘內再升滿 evolved；選擇頁與 HUD 不得露出英文名稱／說明 |
...
鎖定 32 配方、支援型實際施放、Lv.15 存檔邊界及四格神賜規則。最後仍需執行
