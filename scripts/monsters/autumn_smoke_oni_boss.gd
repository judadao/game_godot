...
func _ready() -> void:
	...
	var shoulder_positions: Array[Vector2] = [
		Vector2(-88, -675), Vector2(88, -675),
		Vector2(-88, -610), Vector2(88, -610),
		Vector2(-88, -545), Vector2(88, -545),
	]
	...
	for index in mini(arm_pivots.size(), shoulder_positions.size()):
		...
		hand.position = Vector2(side * 74.0, -8.0)
...
func _process(delta: float) -> void:
	_puppet_time += delta
	if jaw_pivot != null:
		jaw_pivot.rotation = deg_to_rad(6.0 + sin(_puppet_time * 2.2) * 5.0)
	if eye_fire != null:
...
func _build_guardian_archetype() -> Resource:
	...
	result.set("visual_scale", Vector2(1.15, 1.15))
	return result
