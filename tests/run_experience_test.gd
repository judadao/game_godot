...
func _init() -> void:
	...
	_expect(run.experience_required == 100, "Run XP threshold must start at 100 so the opening horde cannot instantly level the player.")
	_expect(int(run.call("add_experience", 99)) == 0, "Sub-threshold XP must not queue a level.")
	_expect(int(run.call("add_experience", 1)) == 1, "Crossing the threshold must queue one level.")
	_expect(run.level == 2 and run.experience_required == 155, "Level two threshold must follow ceil(previous * 1.30 + 25).")
