...
func _init() -> void:
	...
	_expect(run.experience_required == 30, "Run XP threshold must start at 30 for a fast opening level.")
	_expect(int(run.call("add_experience", 29)) == 0, "Sub-threshold XP must not queue a level.")
	_expect(int(run.call("add_experience", 1)) == 1, "Crossing the threshold must queue one level.")
	_expect(run.level == 2 and run.experience_required == 48, "Level two threshold must follow ceil(previous * 1.25 + 10).")
