...
func _run() -> void:
	...
	_expect(float(state.get("surface_arc_radius_scale", 0.0)) >= 1.35, "Residual current must spread beyond the target silhouette instead of clinging to one tiny body ring.")
	_expect(float(state.get("residual_opacity_scale", 1.0)) <= 0.75, "Wider residual current must remain translucent enough to preserve the enemy silhouette.")
	...
	_expect(String(impact_state.get("sky_origin_mode", "")) == "viewport_top", "Final lightning must originate above the visible battlefield rather than a short local offset.")
	_expect(String(impact_state.get("strike_direction", "")) == "top_to_bottom", "Final lightning must reveal downward from sky to ground.")
	_expect(bool(impact_state.get("descending_reveal", false)), "Final lightning must use a descending head before ground contact.")
	_expect(float(impact_state.get("sky_strike_vertical_span", 0.0)) >= 560.0, "Final lightning needs enough vertical span to read as a bolt from the sky.")
