...
func _run() -> void:
	...
	if effect != null:
		...
		if effect.has_method("play_series"):
			...
			_expect(int(basic_state.get("object_count", 0)) == 1, "Basic feather playback must render one sacred feather.")
			_expect(int(advanced_state.get("path_count", 0)) == 1 and int(advanced_state.get("object_count", 0)) > 1, "Advanced feather playback must form one populated lane.")
...
func _finish() -> void:
	if _failures == 0:
		print("PASS: skill-series VFX grows one reusable object through count and paths")
