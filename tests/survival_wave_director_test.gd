...
func _run() -> void:
	...
	horde_director.set("survival_duration", 360.0)
	...
	_expect(
		horde_director.get("scheduled_boss_times") == [60.0, 180.0, 300.0]
			and int(horde_director.call("_boss_count_for_time", 60.0)) == 1
			and int(horde_director.call("_boss_count_for_time", 180.0)) == 1
			and int(horde_director.call("_boss_count_for_time", 300.0)) == 2
			and int(horde_director.call("_boss_count_for_time", 330.0)) == 3,
		"Boss pressure must land at 1:00, return at 3:00, become plural at 5:00, and peak in the final rush."
	)
	var pre_final_rush_growth_events := 0
	for elite_time_variant in horde_director.get("scheduled_elite_times") as Array:
		if float(elite_time_variant) < 330.0:
			pre_final_rush_growth_events += 1
	for boss_time_variant in horde_director.get("scheduled_boss_times") as Array:
		var boss_time := float(boss_time_variant)
		if boss_time < 330.0:
			pre_final_rush_growth_events += int(
				horde_director.call("_boss_count_for_time", boss_time)
			)
	_expect(
		pre_final_rush_growth_events == 9,
		"Before the final rush, five elite and four Boss rewards must sustain rapid six-minute growth."
	)
	...
	_expect(
		...
		"At 1:30 a ten-second super horde must sharply raise density and XP drops after the 1:00 minor Boss."
	)
	...
	var minor_boss: Node
	for enemy in director.call("get_active_enemies") as Array:
		if String(enemy.get_meta("encounter_archetype_id", "")) == "guardian":
			minor_boss = enemy
			break
	_expect(
		minor_boss != null
			and String(minor_boss.get_meta("survival_boss_rank", "")) == "minor",
		"The first scheduled Boss must be explicitly marked as the smaller one-minute Boss."
	)
