...
func _run() -> void:
	...
	await process_frame
	await process_frame
	_expect(not paused, "Resolving the only growth page must resume gameplay.")
	_expect(run.pending_level_ups == 0, "A confirmed EXP growth page must consume one pending level.")
	game.queue_free()
	await process_frame
	if _failures == 0:
