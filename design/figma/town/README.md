...
- `town_base_building_landmark_map_review.svg`：網頁版 Figma 的精簡快速匯入檔；
  ...
  中央大秋樹、MaterialYard-style Base v5 不滅熔爐與傳送門、Base 建築與
...
- `b2_front_style_candidates/`：目前八個嚴格正面朝向、低 AI 痕跡的透明
  ...
  不滅熔爐與戰鬥傳送門的 `*_base_v5.png` 是目前核准來源，
...
- `b2_front_style_candidates/eternal_flame_animation/`：核准的 4×2 火焰與符文
  透明動畫表；火焰 v2 使用固定火根、圓弧外焰與錯相的黃／橙／紅三段大色塊，
  `tools/prepare_town_eternal_flame_animation.py` 會可重現地切成 runtime
  8 幀素材。火焰與符文不可重新烘焙進 Base v5。
- `b2_front_style_candidates/battle_portal_animation/`：核准的 Base v5 chroma
  原稿，以及 core/highlight 兩組 4×3、12 幀手繪漩渦來源；
  `tools/prepare_town_battle_portal_assets.py` 會可重現地產生門洞遮罩與完整
  暗紫 underpaint，並把 core 來源量化／派生為與 Base 對位的 core/highlight
